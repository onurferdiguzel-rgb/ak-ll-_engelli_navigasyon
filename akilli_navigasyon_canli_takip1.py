...
def generate_direction_info(path):
    """
    directions: panelde görünen/tek tek okunabilen talimatlar
    walk_steps: canlı yaya takipte 20 m kala okunacak dönüş/varış noktaları
    """
    if len(path) < 2:
    ...
    for i in range(1, len(path) - 1):
        ...
        if abs(angle) > 35:
            ...
            if angle > 0:
                turn_text = "Sağa dön"
                live_text = "20 metre sonra sağa dönün"
            else:
                turn_text = "Sola dön"
                live_text = "20 metre sonra sola dönün"
